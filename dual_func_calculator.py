...
# Этот калькулятор был изменён для получения корректных результатов на модели
...
#Расчёт был изменён тут
    def dual_func_value(self, times):
        #print(self.phi_big_oracle.func(times))
        #print(self.h_oracle.func(times))
        return self.phi_big_oracle.func(1.7*times+0.3*self.freeflowtimes) + self.h_oracle.func(1.7*times+0.3*self.freeflowtimes)
        #return self.phi_big_oracle.func(times) + self.h_oracle.func(times)
    
    def primal_func_value(self, flows):
        #print(self.h_oracle.conjugate_func(flows))
        #return self.h_oracle.conjugate_func(0.3*flows+0.7*self.capacities)
    ...
    def get_times(self, flows):
        #print(flows)
        #print(self.freeflowtimes * (1.0 + self.rho * np.power(flows / self.capacities, 1.0 / self.mu)))
        return self.freeflowtimes * (1.0 + self.rho * np.power(flows / self.capacities, 1.0 / self.mu))

    def get_t_dot(self, flows):
        #print(self.freeflowtimes * self.rho * np.power(flows / self.capacities, 1.0 / self.mu) / (flows * self.mu))
        mylist = self.freeflowtimes * self.rho * np.power(flows / self.capacities, 1.0 / self.mu) / (flows * self.mu)
        mylist = [0.0 if math.isnan(x) else x for x in mylist]
        return mylist