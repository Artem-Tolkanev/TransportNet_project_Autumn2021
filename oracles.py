...
class HOracle(BaseOracle):
    ...
    def func(self, t_parameter): 
        ...
        if self.mu == 0:
            ...
            res = self.capacities * (t_parameter - self.freeflowtimes) * \
                                      (np.maximum(t_parameter - self.freeflowtimes, 0.0) / \
                                       (self.rho * self.freeflowtimes)) ** self.mu
            h_func = np.sum([0.0 if math.isnan(x) else x for x in res]) / (1.0 + self.mu)
        return h_func
    ...
    def prox(self, grad, point, A):
        ...
        #Позволяем работать с рёбрами, у которых время прохождение равно 0
        ...
        x = newton(x_0_arr = x_0_arr, a_arr = a_arr, mu = self.mu)
