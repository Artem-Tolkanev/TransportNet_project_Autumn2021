...
importlib.reload(ustm)
importlib.reload(ugd)
importlib.reload(sd)
...
class Model:
    def __init__(self, graph_data, graph_correspondences, total_od_flow, mu = 0.25, rho = 0.15):
        ...
        self.mu = mu
        #Переопределяем rho, для учёта штрафов
        self.rho = rho * (mu + 1) / mu
    ...
    def _index_nodes(self, graph_table, graph_correspondences):
        ...
        first_unique = np.unique([table['init_node'][table['init_node_thru'] == True]])
        second_unique = np.unique([table['term_node'][table['term_node_thru'] == True]])
        through_nodes = np.unique(np.concatenate([first_unique, first_unique]))
